...
def draw_boxes(img, detection):
    ...
    for box in de:
        img = cv2.rectangle(img, (box[0], box[1]),
                            (box[2], box[3]), colors[box[5]], thick)
...
class Detective(object):
    ...
    def detect(self, img):
        ...
        if self.catching:
            ...
            # print(self.biker_nohel_count)
        return boxes, biker_which_has_nohel  # for camera drawing box
...
class Filter(object):
    ...
    def save_img(self, img):
        ...
        # filename = r"Home-GUI\static\img\crops\crop.jpg"
    ...
    def run(self):
        ...
        def recheck(img, thresh=0.9):
            ...
            print("No of violations: ",nohel_count, "No of bikes: ", biker_count)
            # if nohel_count == 1:
            #     cv2.imshow("Single No Helmet",img)
            #     cv2.waitKey(1)
            # if nohel_count >=2:
            #     cv2.imshow("Multiple No Helmet",img)
            #     cv2.waitKey(1)
            # if nohel_count >= 1:
            #     for box in boxes:
            #         crop_img = img[int(box[1]):int(box[3]), int(box[0]):int(box[2])]
            #         results = model_plates(img) #ADDED
        ...
        while not waitingQ.empty():
            ...
            for biker in bikers:
                padding = 10
                xmin, ymin, xmax, ymax, _, __ = get_padding(
                    biker, img_w, img_h, padding=padding)
                biker_img = img[ymin:ymax, xmin:xmax]

                thresh = ((biker[3]-biker[1])/(ymax - ymin)) * \
                    ((biker[2]-biker[0])/(xmax-xmin)) - 0.2
